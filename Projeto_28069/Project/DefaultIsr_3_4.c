...
interrupt void ADCINT1_ISR(void)				// PIE1.1 @ 0x000D40  ADCINT1
{
	...
	static float ref = 1;
	static float prev = 1.5;
	static float aux = 0;
	static float ADC_volts = 0;
	// Entradas
	static float amostra = 0;
	static float U = 0;
	static float U1 = 0;
	static float U2 = 0;
	// Erro
	...
	GpioDataRegs.GPASET.bit.GPIO18 = 1;		// Pino de ativacao de rotina
//--- Manage the ADC registers     
	AdcRegs.ADCINTFLGCLR.bit.ADCINT1 = 1;		// Clear ADCINT1 flag

//--- Read the ADC result
	*AdcBufPtr++ = 	AdcResult.ADCRESULT0;     	// Read the result
	ADC_volts =  AdcResult.ADCRESULT0/1200.0; // Converter Valor lido do ADC para Volts
											  // Fundo de escala 12 bits -> 4096 / 3.3 -> 1241, mas dá errado
...
//--- Brute-force the circular buffer
	...
	//             (1/257 sec/sample)*(1 samples/int)*(x interrupts/toggle) = (0.1 sec/toggle)
	//             ==> x = 25.7
	if(GPIO34_count++ > 25)					//Oscilar ref entre 1 e 1.5 a cada 26 interrupções (100ms)
	{
		aux = ref; // guardar valor atual de ref
		ref = prev; // trocar valor
		prev = aux; // atualizar valor de troca para proximo ciclo
	...
	U2 = U1;
	U1 = U;
	U = 0.52524*U1 + 0.47476*U2 -2.9177*err1 + 1.4111*err2 +2.3267 * err;
	if(0>U) //Zerar U se for negativo.
		U=0;
	EPwm1Regs.CMPA.half.CMPA = PWM_DUTY_CYCLE - (PWM_DUTY_GANHO * (U));  //Volts para Duty Cycle ;


	GpioDataRegs.GPACLEAR.bit.GPIO18 = 1;		// Fim de rotina
}
