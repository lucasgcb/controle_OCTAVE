...
//--- Read the ADC result
	*AdcBufPtr++ = 	AdcaResultRegs.ADCRESULT0;			// Read the result
	ADC_volts =  AdcaResultRegs.ADCRESULT0/1200.0;
...
//             ==> x = 25000
	if(GPIO34_count++ > 25)					// Toggle slowly to see the LED blink
	{
	...
	if(0>U) //Zerar U se for negativo.
		U=0;
	EPwm1Regs.CMPA.bit.CMPA = PWM_DUTY_CYCLE - (PWM_DUTY_GANHO * (U));  //Volts para Duty Cycle ;
...
//---------------------------------------------------------------------
interrupt void CANA_1_ISR(void)						// PIE9.5 @ 0x000DC8  CAN-A interrupt #1
{
	PieCtrlRegs.PIEACK.all = PIEACK_GROUP9;			// Must acknowledge the PIE group
...
//---------------------------------------------------------------------
interrupt void CANA_2_ISR(void)						// PIE9.6 @ 0x000DCA  CAN-A interrupt #2
{
	PieCtrlRegs.PIEACK.all = PIEACK_GROUP9;			// Must acknowledge the PIE group
...
//---------------------------------------------------------------------
interrupt void CANB_1_ISR(void)						// PIE9.7 @ 0x000DCC  CAN-B interrupt #1
{
	PieCtrlRegs.PIEACK.all = PIEACK_GROUP9;			// Must acknowledge the PIE group
...
//---------------------------------------------------------------------
interrupt void CANB_2_ISR(void)						// PIE9.8 @ 0x000DCE  CAN-B interrupt #2
{
	PieCtrlRegs.PIEACK.all = PIEACK_GROUP9;			// Must acknowledge the PIE group
